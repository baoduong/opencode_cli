...
export function Sidebar(props: { sessionID: string; overlay?: boolean }) {
  ...
  const mainCost = createMemo(() =>
    messages().reduce((sum, x) => sum + (x.role === "assistant" ? x.cost : 0), 0),
  )

  const childCost = createMemo(() =>
    tasks().reduce((sum, task) => {
      const msgs = sync.data.message[task.id] ?? []
      return sum + msgs.reduce((s, x) => s + (x.role === "assistant" ? (x as AssistantMessage).cost : 0), 0)
    }, 0),
  )
        ...
        >
          <box flexShrink={0} gap={1} paddingRight={1}>
            ...
            <box>
              ...
              <Show when={childCost() > 0}>
                <text fg={theme.textMuted}>
                  ↳ main ${mainCost().toFixed(2)} + tasks ${childCost().toFixed(2)}
                </text>
              </Show>
            </box>
                  ...
                  <For each={tasks()}>
                    {(task) => {
                      ...
                      const elapsed = createMemo(() => {
                        if (!busy() || !task.time.created) return ""
                        return Locale.duration(now() - task.time.created)
                      })
                      ...
                      const parts = createMemo(() =>
                        (sync.data.message[task.id] ?? []).flatMap((msg) =>
                          (sync.data.part[msg.id] ?? []).filter((p): p is typeof p & { type: "tool" } => p.type === "tool"),
                        ),
                      )
                      const current = createMemo(() => {
                        const last = parts().findLast((x) => (x.state as any).title)
                        if (!last) return undefined
                        return `${Locale.titlecase(last.tool)} ${(last.state as any).title}`
                      })
                      const count = createMemo(() => parts().length)

                      const failed = createMemo(() => {
                        if (busy() || retry()) return false
                        const msgs = sync.data.message[task.id] ?? []
                        const last = msgs.findLast((x) => x.role === "assistant") as AssistantMessage | undefined
                        return last?.error != null && last.error.name !== "MessageAbortedError"
                      })

                      const taskCost = createMemo(() => {
                        const msgs = sync.data.message[task.id] ?? []
                        return msgs.reduce(
                          (sum, x) => sum + (x.role === "assistant" ? (x as AssistantMessage).cost : 0),
                          0,
                        )
                      })
                          ...
                          <box flexDirection="row" gap={1}>
                            ...
                            <text fg={theme.text} wrapMode="none">
                              ...
                              <Show when={elapsed()}>
                                ...
                                <Show when={count() > 0}>
                                  <span style={{ fg: theme.textMuted }}> · {count()} tools</span>
                                </Show>
                              </Show>
                              <Show when={duration()}>
                                <span style={{ fg: theme.textMuted }}> · {duration()}</span>
                              </Show>
                              <Show when={!busy() && taskTokens() > 0}>
                                <span style={{ fg: theme.textMuted }}>
                                  {" "}
                                  · {taskTokens().toLocaleString()}tk
                                </span>
                              </Show>
                              <Show when={!busy() && costLabel()}>
                                <span style={{ fg: theme.textMuted }}> · {costLabel()}</span>
                              </Show>
                            </text>
                          </box>
                          <Show when={busy() && current()}>
                            <text fg={theme.textMuted} wrapMode="none" paddingLeft={3}>
                              ↳ {current()}
                            </text>
                          </Show>
                          <Show when={retry()}>
                            <text fg={theme.warning} wrapMode="none" paddingLeft={3}>
